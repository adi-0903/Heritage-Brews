...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/WhatsApp-Ordering-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" />
</p>
...
To ensure that the "Heritage Aesthetic" is never diluted by outdated backend data, we implemented a **Proprietary Force-Sync Engine**:

- **Automatic Keyword Mapping**: The frontend scans product names (e.g., 'Gulab', 'Kaju', 'Mathri') and proactively overrides backend URLs with high-fidelity local `/images/` specimens.
- **Cache-Busting Protocol**: Implements `?v=heritage` query strings on all media assets to force-refresh visuals across sessions.

---

## 🍬 The Artisanal Portfolio (New Features)

We have expanded the archive beyond brews to include the **Regal Confectionery & Savory Collection**:
...
- **Silver Leaf Kaju Katli**: Diamond-shaped cashew fudge glistening with a layer of pure edible silver.

### 🥨 Savoury Snacks

- **Mathri of the Marwars**: Fenugreek-infused flaky crackers, a standard for every merchant caravan.
- **Kachori of the Kachwahas**: Spiced lentil-filled crisp pastry, once a staple of the Jaipur court.
- **Saffron-Salted Cashews**: Hand-selected jumbo kernels roasted with premium Kashmiri Saffron.
- **Rajputana Mathania Peanuts**: Slow-roasted with sun-dried Rajasthani Mathania chilies.
...
- **Frontend**: **React 19 + Vite** implementation featuring custom CSS "sweep" animations and cinematic layouts.
- **Backend**: **Django 5.1 + REST Framework** providing a secure vault for the catalog and patron data.
- **Order Routing**: **WhatsApp Protocol Integration** that converts digital carts into formatted, royal order decrees sent directly to the estate.
...
python manage.py seed_catalog # CRITICAL: Inscribes the Royal Assets
